...
namespace grupo1___github
{
    public partial class Form12 : Form
    {
        ...
        private List<Arista> RutaCorta = new List<Arista>();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            // Algoritmo de Floyd-Warshall para calcular la ruta más corta
            for (int k = 0; k < n; k++)
            {
                for (int i = 0; i < n; i++)
                {
            ...
            // Resaltar ruta más corta del primer vértice al último
            RutaCorta.Clear();
            int inicio = 0;  // Cambiar si es necesario
            int fin = n - 1; // Cambiar si es necesario

            while (inicio != fin)
            {
                var verticeA = vertices[inicio];
                var verticeB = vertices[next[inicio, fin]];
                var arista = aristas.FirstOrDefault(a =>
                    (a.VerticeA == verticeA.Id && a.VerticeB == verticeB.Id) ||
                    (a.VerticeA == verticeB.Id && a.VerticeB == verticeA.Id));
                ...
                if (arista != null)
                {
                    RutaCorta.Add(arista);
                }
                inicio = next[inicio, fin];
            }
            // Solo mantenemos las aristas de la ruta más corta
            aristas = new List<Arista>(RutaCorta);
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var arista in aristas)
            {
                ...
                if (verticeA != null && verticeB != null)
                {
                    var pen = RutaCorta.Contains(arista) ? Pens.Blue : Pens.Black;
                    graphics.DrawLine(pen, verticeA.Posicion, verticeB.Posicion);
